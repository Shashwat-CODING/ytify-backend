class YouTubeIClient {
    ...
    async getTrending() {
        ...
        try {
            ...
            // Get all items from Trending as songs (they're all music content)
            ...
            const trendingSongs = trendingItems
                .map(item => this._normalizeResponsiveListItem(item))
                .filter(item => item.id);

            // 2. Trending Music Videos (from "New music videos") - these go to videos array
            ...
            // Videos - from "New music videos" section
            const videos = newMusicVideos.slice(0, 20);

            // Songs - all trending items (limit to 20)
