class YouTubeIClient {
    ...
    async getTrending() {
        ...
        try {
            ...
            // 1. Process "Trending" section
            // This section contains a mix of Songs and Videos.
            // We differentiate them by checking the subtitle (Col 1).
            // Videos usually have "Artist • Views" (e.g. "Mangli • 6M views").
            // Songs usually have "Artist" or "Artist • Album" (e.g. "Anirudh Ravichander").
            ...
            const trendingSongs = [];
            const trendingVideosFromSection = [];
            ...
            for (const item of trendingItems) {
                ...
                // Check if subtitle (Col 1) contains "views"
                // In _normalizeResponsiveListItem, we parse flex columns.
                // But we need to check the raw item or the normalized 'views' field?
                // The normalized 'views' field is populated if "views" text is found.
                // So if normalized.views is present, it's likely a video.
                // HOWEVER, some songs might have views? usually YTM songs don't show views in the list, only videos do.

                // Let's rely on the normalized 'views' field which I populated from the subtitle.
                if (normalized.views && (normalized.views.includes('views') || normalized.views.includes('view'))) {
                    isVideo = true;
                }

                // Also check navigation endpoint if available (but normalized doesn't have it)
                // We can check item.endpoint directly here if needed.

                if (isVideo) {
                    trendingVideosFromSection.push(normalized);
                } else {
                    trendingSongs.push(normalized);
                }
            }

            // 2. Trending Music Videos (from "New music videos")
            const videoSection = explore.sections.find(s => (s.title?.text || s.header?.title?.text) === 'New music videos');
            const newMusicVideos = (videoSection?.contents || [])
                .map(item => this._normalizeResponsiveListItem(item))
                .filter(item => item.id);

            // Combine videos
            const videos = [...trendingVideosFromSection, ...newMusicVideos].slice(0, 20);

            // Songs
            const songs = trendingSongs.slice(0, 20);

            // 3. Trending Playlists
